...
const tickets = [
  { id: '#SR-9921', title: 'Whale Wallet Re-keying', detail: 'Illustrative concierge transcript preserved from the design pass.', state: 'PREVIEW', active: true },
  { id: '#SR-9844', title: 'L2 Gas Optimization API', detail: 'Sample queue item only.', state: 'SAMPLE', active: false },
  { id: '#SR-9012', title: 'Treasury Migration 0x...F32', detail: 'Archived mock engagement.', state: 'ARCHIVE', active: false },
] as const

export function SupportAccessPage() {
  return (
    <ConsoleLayout
      eyebrow="Demo-grade support artifact"
      title="Direct Support Preview"
      description="A tri-pane concierge workspace preserved as a UI-only submission preview. No live messaging, ticketing, or attachment workflow is wired in today's MVP."
      ...
      <div className="flex min-h-[calc(100vh-220px)] flex-col overflow-hidden border border-white/6 bg-aegis-foundation xl:flex-row">
        <section className="w-full border-r border-white/6 bg-aegis-shell xl:w-80">
          <div className="p-6">
            ...
            <p className="mt-1 text-xs uppercase tracking-[0.18em] text-aegis-text-muted">Sample engagements</p>
            <div className="mt-4 rounded-xl border border-aegis-warning/20 bg-aegis-warning/8 p-4 text-xs leading-6 text-aegis-text-muted">
              This page is a stitched concierge preview only. Use `/help-center` for truthful documentation links and `/evaluation-dashboard` for the live demo flow.
            </div>
          </div>
          <div className="space-y-2 px-3 pb-4">
            {tickets.map((ticket) => (
              <button
                key={ticket.id}
                className={`w-full rounded-xl border-l-2 p-4 text-left transition-all ${ticket.active ? 'border-aegis-primary bg-aegis-highest/40' : 'border-transparent hover:bg-white/[0.03]'} ${ticket.state === 'ARCHIVE' ? 'opacity-60' : ''}`}
                type="button"
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  <span className={`font-headline text-sm font-semibold ${ticket.active ? 'text-aegis-primary' : 'text-aegis-text'}`}>{ticket.id}</span>
                   <Badge tone={ticket.state === 'PREVIEW' ? 'warning' : ticket.state === 'SAMPLE' ? 'info' : 'neutral'}>{ticket.state}</Badge>
                </div>
                ...
                <p className="mt-1 text-xs text-aegis-text-muted">{ticket.detail}</p>
                 {ticket.active ? (
                   <div className="mt-3 flex items-center justify-between">
                    ...
                    <p className="text-[10px] italic text-aegis-text-muted">Preview timestamp</p>
                  </div>
                ) : null}
              </button>
            ))}
          ...
          <div className="border-t border-white/6 p-4">
            <button className="btn-secondary w-full justify-center" disabled type="button">Ticketing coming soon</button>
          </div>
        </section>
        ...
        <section className="flex min-w-0 flex-1 flex-col bg-aegis-foundation">
            <div className="flex h-20 items-center justify-between border-b border-white/6 bg-aegis-foundation/80 px-6 backdrop-blur-md lg:px-8">
              ...
              <div className="relative">
                ...
                <div>
                  <h3 className="font-headline text-lg font-bold text-aegis-text">Sample concierge transcript</h3>
                  <p className="text-[11px] uppercase tracking-[0.18em] text-aegis-primary">Illustrative preview only</p>
                </div>
              </div>
            <div className="flex items-center gap-3 text-aegis-text-muted">
              <button className="rounded-lg p-2" disabled type="button"><Icon name="videocam" /></button>
              <button className="rounded-lg p-2" disabled type="button"><Icon name="call" /></button>
              <button className="rounded-lg p-2" disabled type="button"><Icon name="more_vert" /></button>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-8 overflow-y-auto px-6 py-8 lg:px-8">
            <div className="flex items-center gap-4">
              <div className="h-px flex-1 bg-white/8" />
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-aegis-text-muted">Illustrative transcript</span>
              <div className="h-px flex-1 bg-white/8" />
            </div>
            ...
            <div className="flex items-center gap-2 text-aegis-primary/60">
              <div className="flex gap-1">
               ...
               <span className="text-[10px] italic">Preview typing indicator...</span>
            </div>
          </div>
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-1 text-aegis-text-muted">
                    <button className="rounded-lg p-1.5" disabled type="button"><Icon name="attach_file" className="text-sm" /></button>
                    <button className="rounded-lg p-1.5" disabled type="button"><Icon name="image" className="text-sm" /></button>
                    <button className="rounded-lg p-1.5" disabled type="button"><Icon name="password" className="text-sm" /></button>
                  </div>
                  <span className="text-[10px] font-mono text-aegis-text-muted/65">Composer state: preview only</span>
                </div>
              </div>
              <button className="rounded-lg bg-aegis-primary p-3 text-zinc-950 opacity-50" disabled type="button">
                <Icon name="send" />
              </button>
            </div>
          </div>
        ...
        <aside className="w-full border-l border-white/6 bg-aegis-shell p-6 xl:w-80">
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="rounded-xl border border-aegis-primary/12 bg-aegis-primary/6 p-4 text-sm leading-6 text-aegis-text-muted">
               Illustrative case detail card only. Use the evaluation routes for the real MVP evidence.
             </div>
          </div>
        </aside>
      </div>
